...
class User(UserMixin, db.Model):
    ...
    status = db.Column(db.String(20), default='created', nullable=False)  # created, pending, deleted
    is_active = db.Column(db.Boolean, default=True, nullable=False)
    is_default_account = db.Column(db.Boolean, default=False, nullable=False)
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(GMT_PLUS_7))
    ...
    def __init__(self, username, password, role='viewer', email=None, full_name=None, status='created'):
        ...
        # Set pending expiry to 7 days from now for pending registrations
        if status == 'pending':
            self.pending_expires_at = datetime.now(GMT_PLUS_7) + timedelta(days=7)
    ...
    def is_pending_expired(self):
        ...
        if self.status == 'pending' and self.pending_expires_at:
            return datetime.now(GMT_PLUS_7) > self.pending_expires_at
        return False
